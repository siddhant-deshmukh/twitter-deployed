...
const FeedTweetComponent = ({ tweet , updateTweet, pageNum, indexNum}: { 
  ...
  return (
    <div 
      ...
      <Link href={`#`} className="w-[72px]  h-fit px-3 user-link">
        {
          tweet.authorDetails.avatar
          && <img className="rounded-full bg-black hover:opacity-70 w-12 h-12 user-link" src={tweet.authorDetails.avatar} />
        }
        {
          !tweet.authorDetails.avatar && <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.0} stroke="currentColor" className="w-10 h-10 user-link">
      ...
      <div className="w-full flex flex-col ">
        <div className="flex items-center space-x-1">
          ...
          <Link href={`/uesr/${tweet.author}`} className="user-link w-fit text-sm  text-gray-400">@{tweet.authorDetails.user_name}</Link>
        </div>
