        ...
        <ul className="flex justify-between text-gray-500">
          <li className="w-full">
            <button className="w-fit items-center flex justify-start space-x-1 pr-4 py-2  group hover:text-blue-700 tweet-btn">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.0} stroke="currentColor" className="p-1.5 w-7 h-7 text-gray-500 rounded-full group-hover:text-blue-700 group-hover:bg-blue-100">
          ...
          <li className="w-full">
            <button
              className={`w-fit ${(tweet.have_retweeted) ? 'text-green-500' : ''} items-center flex justify-start space-x-1 pr-4 py-2  group hover:text-green-700 tweet-btn`}
              onClick={(event: React.MouseEvent<HTMLButtonElement>) => {
                event.preventDefault();
                updateTweet('retweeted')
              }}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.0} stroke="currentColor" className={`p-1.5 w-7 h-7 ${(tweet.have_retweeted) ? 'text-green-500' : 'text-gray-500'} rounded-full group-hover:text-green-700 group-hover:bg-green-100`}>
          ...
          <li className="w-full">
            <button
              className={`w-fit ${(tweet.have_liked) ? 'text-red-700' : ''} items-center flex justify-start space-x-1 pr-4 py-2  group hover:text-red-700 tweet-btn`}
              onClick={(event: React.MouseEvent<HTMLButtonElement>) => {
                event.preventDefault();
                updateTweet('liked')
              }}>
              <svg xmlns="http://www.w3.org/2000/svg" fill={(tweet.have_liked) ? 'rgb(256 68 68)' : 'none'} viewBox="0 0 24 24" strokeWidth={2.0} stroke="currentColor" className={`p-1.5 w-7 h-7 ${(tweet.have_liked) ? 'text-red-700' : 'text-gray-500'}  rounded-full group-hover:text-red-700 group-hover:bg-red-100`}>
          ...
          <li className="w-full">
            <button
              className="w-fit items-center flex justify-start space-x-1 pr-4 py-2  group hover:text-blue-700 tweet-btn"
              onClick={(event) => {
                console.log("Here")
