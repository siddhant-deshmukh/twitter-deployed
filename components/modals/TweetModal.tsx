...
const TweetModal = () => {
    ...
    return (
        <div className='fixed left-0 right-0 z-20 bg-black bg-opacity-50 w-screen h-screen pt-14 px-2'>
            <div className='w-full max-w-[600px] h-fit bg-white mx-auto rounded-2xl overflow-hidden'>
                <header className='flex items-center justify-between   w-full'>
                    <button
                        className='w-10 h-10 text-lg hover:bg-gray-200 m-2 rounded-full '
                        onClick={(event) => {
                ...
                <main className='min-h-[200px]  relative flex'>
                    <FeedTweetEditor closeModal={closeModal}/>
                </main>
            </div>
        </div>
    )
}
function FeedTweetEditor({closeModal}:{
    closeModal: () => void
}) {
