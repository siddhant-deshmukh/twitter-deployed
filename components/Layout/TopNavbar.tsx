...
function TopNavBar({ authState }: { authState: IUser }) {
    ...
    useEffect(() => {
        ...
        const main_ele = document.querySelector('#main-element')
        const top_nav_bar = document.getElementById('top_nav_bar')
        var prevPos = 0;
        const hideTopNavBar = (event : Event) => {
            if (timeoutID) clearTimeout(timeoutID);
            timeoutID = setTimeout(() => {
                if(!event.target || !top_nav_bar) return

                // console.log("Meow",prevPos,event.target.scrollTop)
                // @ts-ignore
                if(prevPos > event.target.scrollTop - 5){
                    // console.log('0',top_nav_bar)
                    top_nav_bar.style.top = "0px";
                }else{
                    top_nav_bar.style.top = "-50px";
                    // console.log('-10',top_nav_bar)
                }
                // @ts-ignore
                prevPos = event.target.scrollTop;
        ...
        if (main_ele) {
            main_ele.addEventListener('scroll', hideTopNavBar)
        }

        return () => {
            if (main_ele) main_ele.removeEventListener('scroll', hideTopNavBar)
        }
    }, [])

    if (pathname === '/home') {
        return (
            <div id="top_nav_bar" className="fixed z-50 flex items-center bg-white dark:bg-black justify-between bg-opacity-80 h-14 duration-100 ease-in-out  w-full sm:hidden top-0 left-0">
                <button
                    onClick={(event) => {
